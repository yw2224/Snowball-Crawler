...
for index, item in pairs(decode(args)) do
   ...
   local timelineSuffix = (item.tls and tostring(item.tls)) or ''
   local timelinePrefix = (item.tlp and tostring(item.tlp)) or ''
   local timelineHash = timelinePrefix .. key .. timelineSuffix
